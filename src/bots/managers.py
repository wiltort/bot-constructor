...
class BotManager(models.Manager):
    ...
    def get_all_bots_with_steps(self):
        ...
        """
        bots = (
            self.select_related('current_scenario')
                .prefetch_related('current_scenario__steps')
        )
        return bots
    ...
    def get_steps_count(self, bot_id=None):
        ...
        if bot_id:
            ...
            scenario_ids = list(
                self.filter(current_scenario__isnull=False)
                    .values_list('current_scenario_id', flat=True)
            )
